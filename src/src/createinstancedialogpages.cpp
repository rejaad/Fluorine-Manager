...
QString compactPathForDescription(const QString& path)
{
  const QString nativePath = QDir::toNativeSeparators(path);
  const QString homePath   = QDir::toNativeSeparators(QDir::homePath());

  if (nativePath.startsWith(homePath, Qt::CaseInsensitive)) {
    return "~" + nativePath.mid(homePath.size());
  }

  return nativePath;
}

void tuneTypeButtonHeight(QCommandLinkButton* button)
{
  // Qt can underestimate command-link height on Linux for long descriptions.
  button->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::MinimumExpanding);
  button->setMinimumHeight(std::max(button->minimumSizeHint().height(), 110));
}
    ...
    : Page(dlg)
{
  ...
  ui->createGlobal->setDescription(ui->createGlobal->description().arg(
      compactPathForDescription(InstanceManager::singleton().globalInstancesRootPath())));

  ui->createPortable->setDescription(ui->createPortable->description().arg(
      compactPathForDescription(InstanceManager::singleton().portablePath())));
